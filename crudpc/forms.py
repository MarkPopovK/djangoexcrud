...
class ProcessorForm(ModelForm):
    ...
    def clean(self):
        ...
        if not Processor.objects.filter(model__icontains=model).count():
            self.add_error('model', notexist)
        if speed and not Processor.objects.filter(speed=speed).count():
...
class RAMForm(ModelForm):
    ...
    def clean(self):
        ...
        speed = cleaned_data.get('speed')
        notexist = 'No such entries!'
        if model and not RAM.objects.filter(model__icontains=model).count():
            self.add_error('model', notexist)
        if speed and not RAM.objects.filter(speed=speed).count():
            self.add_error('speed', notexist)
